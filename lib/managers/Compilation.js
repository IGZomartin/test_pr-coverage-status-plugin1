...
const publicToken = require('../util/public_token');
...
function createCompilation(productId, compilationData, callback) {
  ...
  async.parallel({
    ...
    cloudManager.createUpload({file: compilationPath}, function(err, signedUrl) {
      ...
      compilationData.publicToken = publicToken.getPublicToken();

      product.compilations.push(compilationData);
      product.save(function(err, modifiedProduct) {
        if (err) return callback(new errors.BadRequestError('Could not create compilation'));

        // Return the whole compilation object for the frontend to have it more accessible. CompilationId is redundant
        // but has not been removed for compatibility purposes i.e. upload compilation scripts and other frontend functions
        let createdCompilation = _.where(modifiedProduct.compilations, { compilationId: compilationData.compilationId})[0];
        return callback(null, {compilationId: compilationData.compilationId, compilation: createdCompilation, url: signedUrl.url});
      });
    });
  });
}
...
function deleteCompilation(productId, compilationId, callback) {
  ...
  Product.findOne({_id: productId, compilations: {$elemMatch: {compilationId: compilationId}}}, function(err, product) {
    ...
    let compilationPath = compilationInfo.filePath;
...
function downloadCompilation(productId, compilationId, publicToken, callback) {
  ...
  Product.findOne({_id: productId, compilations: {$elemMatch: {compilationId: compilationId, uploaded: true}}}, function(err, product) {
    ...
    if (publicToken && compilationInfo.publicToken !== publicToken) {
      return callback(new errors.ForbiddenError('Sorry, you do not have permission to download this app'));
    }
...
function uploadAckCompilation(productId, compilationId, callback) {
  ...
  Product.findOneAndUpdate({ _id: productId, compilations: { $elemMatch: { compilationId: compilationId } } },
    ...
    {new: true}, function(err, product) {
      ...
      NotificationManager.sendAckCompilationEmails(identities, product, compilation, function(err) {
        if (err) return callback(err);
